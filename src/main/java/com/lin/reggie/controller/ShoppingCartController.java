...
public class ShoppingCartController {
    ...
    @Transactional
    @PostMapping("/sub")
    public Result<String> deleteOneInCart(@RequestBody ShoppingCart shoppingCart){
        log.info("dishid:{}",shoppingCart);
        Long dishId = shoppingCart.getDishId();
        Integer number = shoppingCart.getNumber();
        int currentId = number -1;
        if (number> 0 && currentId != 0) {
            --number;
            ...
            return Result.success("减一，已清空");
        }
        LambdaQueryWrapper<ShoppingCart> queryWrapper = new LambdaQueryWrapper<>();
        queryWrapper.eq(ShoppingCart::getDishId,dishId);
        shoppingCartService.update(shoppingCart,queryWrapper);
        return Result.success("减一");
    }
}